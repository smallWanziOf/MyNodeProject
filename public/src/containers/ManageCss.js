...
export default class ManageCss extends Component{
  constructor(props) {
    super(props);
    this.state = {
...
class PublishArticle extends Component{
  ...
  textFieldChange = () => {
    ...
    if(!title || !summary || !author || !article || !articleDate){
      ...
      fetch(`${TANGJG.HOST}/method/publishCssArticle`,{
        method:'POST',
        headers: {
...
class HistoryArticle extends Component{
  ...
  fetchArticle = () => {
    let {page} = this.state;
    fetch(`${TANGJG.HOST}/method/queryCssArticle?page=${page}`,{
      method:'get',
      headers: {
        'Content-Type': 'application/json'
      },
  ...
  handleDelete = (e) => {
    ...
    fetch(`${TANGJG.HOST}/method/deleteCssArticle?id=${value}`,{
      method:'get',
      headers: {
        'Content-Type': 'application/json'
      },
              ...
              dataSource.map((item,i)=>{
                return (
                  <TableRow key={i}>
                    ...
                    <TableRowColumn>
                      <Link to={{pathname:"/editCssArticle",state:item.ID}}>编辑</Link>|
