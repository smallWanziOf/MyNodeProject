 ...
 * 发表新的HTML文章
 */
export default class EditHtmlArticle extends Component{
  ...
  queryHtmlArticle = () => {
    fetch(`${TANGJG.HOST}/method/queryHtmlAtticleFromId?id=${this.state.value}`,{
      credentials: 'include',
      method:'get',
      headers:{
  ...
  textFieldChange = () => {
    ...
    if(!title || !summary || !author || !article || !articleDate){
      ...
      fetch(`${TANGJG.HOST}/method/EditHtmlArticleFromId?id=${value}`,{
        credentials: 'include',
        method:'POST',
        headers: {
          'Content-Type': 'application/json'
      ...
      .then(json=>{
        if(json.code == "S"){
          ...
          this.clearAllfields();
          setTimeout(()=>{this.props.history.push('/manageHtml')},2000)
        }else{
          this.setState({
            open:true,
            error:'修改失败'+json.msg
